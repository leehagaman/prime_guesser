...
def get_model_primes(max_new_tokens, temperature, seed_primes):
    # running this in a subprocess, since my cuda environment exists as the command line python
    env = os.environ.copy()
    command = f"python test.py --max_new_tokens {max_new_tokens} --temperature {temperature} --seed_primes {' '.join(map(str, seed_primes))}"
    result = subprocess.run(command, shell=True, capture_output=True, text=True, env=env)
    lines = result.stdout.splitlines()
    if not lines:
        print("get_model_primes error output:", result.stderr)

    primes_str = lines[1]
    primes = [int(s) for s in primes_str.split(",")[:-1]]
